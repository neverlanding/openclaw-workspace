...
### 核心要求
当助手之间后台沟通完成后，**必须**向组长发飞书消息汇报任务状态。
...
### 2026-03-16 - Star-Office-UI 加载失败（新教训！）

**错误**：办公室视图一直加载不出来，**Phaser 游戏引擎从 CDN 加载超时**。
...
**正确做法**：
```markdown
1. 【检查】部署服务时验证所有外部资源可访问性
   ├─ CDN 资源需有本地备用
   ├─ 关键依赖必须本地存储
   └─ 首次启动验证加载成功

2. 【解决】Phaser 本地化
   - 下载 phaser.min.js 到本地 static/
   - 修改 index.html 使用本地版本
   - 不再依赖外部 CDN
```

**黄金法则**：
> 核心依赖必须本地存储，网络不可靠时服务仍然可用。

**已执行修复**：
- ✅ 下载 Phaser 3.80.1 (1.18MB) 到本地
- ✅ 修改 index.html: `/static/phaser.min.js`
- ✅ 验证加载成功
