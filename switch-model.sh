...
case "$MODEL" in
  qwen|qwen-portal|default)
    ...
    output=$(run_openclaw models set qwen-portal/coder-model)
    echo "$output"
    ;;
  kimi|kimi-k2|kimi-coding)
    echo "切换到 Kimi K2.5 模型..."
    output=$(run_openclaw models set kimi-coding/k2p5)
    echo "$output"
    ;;
  glm|zai)
    echo "切换到 GLM 4.7 模型..."
    output=$(run_openclaw models set zai/glm-4.7)
    echo "$output"
    ;;
  status)
    echo "当前模型状态:"
